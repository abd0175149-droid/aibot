...
describe('★ العنوانُ غير الصالح — القاعدة تُعطّل نفسها', () => {
  ...
  it('★★★ وما ليس عنواناً شكلاً يُرفض — الرفضُ افتراضيٌّ لا القبول', () => {
    /* `X-Forwarded-For` تُزوَّر، ويومَ يُوثَق الجسرُ تصير قيمتُها هي `req.ip`:
       ...
       والنمطُ المكتوب بيد لا يكفي: `[0-9a-f:]+` يقبل `'a'.repeat(50)`
       و`deadbeef` — ولذلك `isIP` من المكتبة، وهو يحدّ الطولَ أيضاً. */
    for (const junk of [
      'not-an-ip', '$(evil)', 'a'.repeat(50), 'deadbeef', '123456789',
      '999.999.999.999', '127.0.0.1, 8.8.8.8', '8.8.8.8:443', '  8.8.8.8  ',
    ]) expect(isUnusableClientIp(junk), junk).toBe(true);
  });

  it('★ والعامّةُ ما زالت تمرّ — فالتشديدُ لم يُطفئ القاعدة', () => {
    for (const ip of ['92.241.36.203', '8.8.8.8', '2a01::1', '::ffff:8.8.8.8']) {
      expect(isUnusableClientIp(ip), ip).toBe(false);
    }
  });
});
