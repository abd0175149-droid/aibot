...
/* ══════════════════════════════════════════════════════════════════════════
   ★ مصطلحٌ واحدٌ لكلّ معنى — في شاشات المالك وحدها.

   العطل مقيسٌ على شاشاتٍ حيّة: «وحدة قراءة» في شاشة البوت، و«توكن» في الساحة
   (ستّةَ عشرَ موضعاً)، و«توكناتُ الذكاء» في الاستهلاك، و«توكن» في المعالج —
   أربعةُ أسماءٍ لرقمٍ واحد، فيظنّها صاحبُ المطعم أربعةَ حسابات. ووضعُ المعرفة
   كان له أربعُ خرائطِ أسماءٍ مستقلّة واسمان للمفهوم نفسِه.

   وأسوأُ صوره مكالمةُ دعم: العميل يقول «يقرأ نصّي كاملاً» والموظّفُ أمامه
   «حقنٌ كامل» — فيُشخّص حالةً غيرَ التي يصفها له.

   ⚠️ والماسحُ يُعمي التعليقات أوّلاً: الإصلاحُ نفسُه يترك تعليقاتٍ تقتبس
      «توكن» و«حقنٌ كامل» و«مِلّي» لتشرح ما استُبدل بها. وماسحٌ لا يُعميها
      يبلّغ عن شرحه هو — وإنذارٌ كاذبٌ واحدٌ أسرعُ طريقٍ إلى إطفاء الحارس.
   ══════════════════════════════════════════════════════════════════════════ */

describe('مصطلحٌ واحدٌ لكلّ معنى — لا مرادفَ في شاشات المالك', () => {
  /* لوحةُ المالك مستثناةٌ عمداً: مفرداتُ المحرّك صحيحةٌ لمن يشغّل المنصّة. */
  const OWNER = files.filter((f) => f.rel.startsWith('app/app/') || f.rel.startsWith('components/'));

  const hits = (pool: typeof files, re: RegExp): string[] => pool.flatMap((f) => stripComments(f.src)
    .split(/\n/)
    .flatMap((line, i) => (re.test(line) ? [`${f.rel}:${i + 1}`] : [])));

  it('الماسحُ يقرأ شاشاتٍ فعلاً', () => {
    expect(OWNER.length).toBeGreaterThan(8);
  });

  it('★★ لا «توكن» بمعنى وحدة القياس في شاشة مالك', () => {
    /* والنمطُ يقصد المعنى المحاسبيّ وحده: «التوكن الدائم» و«توكنٌ مكسور»
       و«توكنه» بيانُ اعتمادٍ عند ميتا يَنسخه المالك من لوحتها هو، فيبقى. */
    expect(hits(OWNER, /توكناً|توكنز|توكنات|unit\s*[:=]\s*['"`]\s*توكن/)).toEqual([]);
  });

  it('★★ والوحدةُ معرَّفةٌ مرّةً واحدةً في المعجم', () => {
    const owners = files.filter((f) => stripComments(f.src).includes('وحدة قراءة')).map((f) => f.rel);
    expect(owners).toEqual(['lib/terms.ts']);
  });
  ...
  it('★★★ واسمٌ واحدٌ لكلّ وضع معرفة — وكانت أربعَ خرائط', () => {
    ...
    /* والمرادفاتُ المتقاعدةُ تختفي من الشجرة كلِّها — ومنها «أساسيات» بلا
       نقطتَين، وهي نسخةُ لوحة المالك وكانت سلسلةً مختلفةً لا تُطابَق. */
    expect(hits(files, /حقنٌ كامل|'هجين'|أساسيات \+ استرجاع/)).toEqual([]);
  });

  it('★ واسمٌ واحدٌ للمفهوم — «وضع القراءة» لا «وضعُ المعرفة»', () => {
    expect(hits(files, /وضعُ المعرفة/)).toEqual([]);
  });

  it('★★ ولا مصطلحَ آلةٍ في شاشة مالك', () => {
    /* `HTTP 429` و`820ms` و«JSON» و«الكاش» و«مِلّي»: كلُّها تصل صاحبَ متجرٍ
       لا يملك فعلاً يفعله بها. والبديلُ يقول العاقبةَ لا الاسم. */
    expect(hits(OWNER, /مِلّي|الكاش|JSON صالح|HTTP \$\{/)).toEqual([]);
  });

  it('★★★ والماسحُ يمسك الشكلَ ويحترم التعليق — وإلّا لم يُثبت شيئاً', () => {
    const bad = { rel: 'x.tsx', src: "const a = \'unit: 'توكن'\';\nconst b = `HTTP ${x}`;" };
    const good = { rel: 'y.tsx', src: '/* شرحٌ يقتبس «توكناً» و«حقنٌ كامل» و«مِلّي» */\nconst c = 1;' };
    expect(hits([bad] as typeof files, /توكن/).length).toBeGreaterThan(0);
    expect(hits([bad] as typeof files, /HTTP \$\{/).length).toBeGreaterThan(0);
    expect(hits([good] as typeof files, /توكناً|حقنٌ كامل|مِلّي/), 'الماسحُ يبلّغ عن شرحه هو')
      .toEqual([]);
  });
});
