 ...
 *
 * وهي أسوأ من رسالةٍ عامّة: تدفع المستخدم إلى الفعل الخطأ. من أخطأ حرفاً في
 * كلمته يُعيد الدخول بالكلمة نفسِها ويدور، ومن أُوقف حسابُه يظنّ العطلَ في
 * الجلسة فيُعيد المحاولة بدل أن يتّصل.
 */
describe('رسالةُ الخادم تصل المستخدم', () => {
  const a = code('lib/api.ts');

  it('★ رسالةُ الخادم تسبق الخريطةَ العامّة — والخريطةُ احتياط', () => {
    /* كان `HUMAN[code] ?? j.error?.message`: الخريطةُ تغلب دائماً، فكلُّ ٤٠٠
       يصير «تحقّق من الحقول» مهما قال الخادم بالضبط ما هو غير الصالح. */
    expect(a).toMatch(/message = j\.error\?\.message \?\? HUMAN\[code\] \?\? message/);
    expect(a, 'الترتيبُ المقلوب باقٍ').not.toMatch(/HUMAN\[code\] \?\? j\.error\?\.message/);
  });

  it('★★ و٤٠١ من نداءٍ بلا توكن ليست «انتهت جلستك»', () => {
    /* هذا الفرعُ كان يلتقط ردَّ `/auth/login` على كلمةٍ خاطئة ويرمي رسالةً
       ثابتةً **بلا أن يقرأ الجسم أصلاً**. */
    expect(a).toMatch(/const hadToken = Boolean\(accessToken\)/);
    expect(a).toMatch(/res\.status === 401 && init\.retry !== false && hadToken/);
  });
});
