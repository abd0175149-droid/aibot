...
describe('★★★ بوّابةٌ لا يجوز أن تتعلّق — والتعلّقُ وقع فعلاً', () => {
  ...
  it('★★★ والتثبيتُ غيرُ تفاعليٍّ — وإلّا كان نجاحاً بلا عمل', () => {
    /* 🔴 pnpm يسأل حين يقرّر إعادةَ بناء `node_modules`:
       ...
       وهذا أسوأُ من التعلّق: تعلّقٌ يُرى، وهذا لا يُرى.
       ووقع فعلاً: بقيت `xlsx@0.18.5` بينما القفلُ يقول 0.20.3.

       ⚠️ والاثنتان معاً لازمتان: `CI=true` تمنع طرحَ السؤال، و`< /dev/null`
          تمنع انتظارَ جوابه لو طُرح. إحداهما بلا الأخرى تُعيد أحدَ العطلَين. */
    const at = gate.indexOf('pnpm@9 install');
    expect(at).toBeGreaterThan(0);
    const call = gate.slice(Math.max(0, at - 200), at + 60);
    expect(call, 'التثبيتُ تفاعليٌّ — سيمضي بلا عملٍ عند EOF').toContain('CI=true');
  });
